...
function renderQuotes(quoteArray) {
    quoteArray.forEach(quote => {
        ...
        <li id=${quote.id} class='quote-card'>
            ...
            <footer class="blockquote-footer">${quote.author}</footer>
            <br>
            <button id=${quote.id} class='btn-success'>Likes: <span id=${quote.id}>${quote.likes.length}</span></button>
...
newQuoteForm.addEventListener('submit', function(e) {
    ...
    saveNewQuoteToDb(newQuoteObj)

    e.target.reset()
})


function saveNewQuoteToDb(newQuoteObj) {
  fetch('http://localhost:3000/quotes', {
    method: 'POST',
    headers: {
        'Content-Type': 'application/json',
...
function renderNewQuote(data) {
    quoteList.innerHTML += `
        <li id=${data.id} class='quote-card'>
            <blockquote class="blockquote">
            <p class="mb-0">${data.quote}</p>
            <footer class="blockquote-footer">${data.author}</footer>
            <br>
            <button id=${data.id} class='btn-success'>Likes: <span id=${data.id}>${0}</span></button>
            <button id=${data.id} class='btn-danger'>Delete</button>
            </blockquote>
        </li>
        `
}
...
quoteList.addEventListener('click', function(e) {
    if (e.target.matches('.btn-danger')) {
        const quoteToRemove = e.target.closest('.quote-card')
        removeQuoteFromDb(quoteToRemove)
        quoteToRemove.remove()
    } else if (e.target.matches('.btn-success')) {
        numToIncrease = parseInt(e.target.innerText.split(' ')[1])
        numToIncrease++
        e.target.querySelector('.likes-span').innerText = "Likes: " + numToIncrease
    }
})
...
function removeQuoteFromDb(quote) {
